...
(defun agent-shell-fork-tree--render ()
  "Draw only the current conversation, preserving its selected endpoint."
  (let* ((inhibit-read-only t) (store agent-shell-fork-tree--store)
         ...
         (visible (agent-shell-fork-tree--visible))
         (current (gethash agent-shell-fork-tree--focus (agent-shell-fork-tree--store-sessions store)))
         (active (mapcar (lambda (r) (map-elt r 'node)) (and current (agent-shell-fork-tree--session-path current))))
         position)
    (unless (gethash agent-shell-fork-tree--selected visible)
      (setq agent-shell-fork-tree--selected 0 agent-shell-fork-tree--selected-session nil))
    (setq header-line-format (format " Fork tree · %d sessions · auto %s · %s "
                                     (length sessions) (if agent-shell-fork-tree-auto-rebuild "on" "off")
    ...
    (let ((stack '((0 "" "" t))))
      (while stack
        (pcase-let* ((`(,id ,prefix ,edge ,last) (pop stack))
                     ...
                     (start (point)))
          (insert prefix edge (if (memq id active) "● " "○ ")
          ...
          (when (and (null agent-shell-fork-tree--selected-session) (= id agent-shell-fork-tree--selected)) (setq position start))
          (dolist (session sessions)
            (when (= id (or (map-elt (car (last (agent-shell-fork-tree--session-path session))) 'node) 0))
              (let* ((start (point)) (sid (agent-shell-fork-tree--session-id session)))
                (insert prefix "  ↳ " (propertize (replace-regexp-in-string "[\n\r]+" " " (agent-shell-fork-tree--session-title session)) 'face 'font-lock-function-name-face)
                        "  [" sid "]" (if (equal sid agent-shell-fork-tree--focus) "  current" "")
                ...
                (when (equal sid agent-shell-fork-tree--selected-session)
                  (setq position start agent-shell-fork-tree--selected id)))))
          (dolist (child (reverse children))
            (let ((child-last (equal child (car (last children)))))
              (push (list child (concat prefix (if (string-empty-p edge) "" (if last "   " "│  ")))
                          (if (> (length children) 1) (if child-last "└─ " "├─ ") "") child-last) stack))))))
    ...
    (let ((cancel
           (agent-shell-fork-tree--scan
            ...
            (lambda (_store status)
              (when (buffer-live-p view)
                (with-current-buffer view
                  (setq agent-shell-fork-tree--status (concat (when full "Full rebuild · ") status))
                  (agent-shell-fork-tree--render))))
            (lambda (result error-text cancelled)
              (when (and (not error-text) (not cancelled))
                ...
                (agent-shell-fork-tree--save result)
